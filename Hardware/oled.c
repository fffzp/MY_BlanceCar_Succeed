...
void OLED_ShowFloat(uint8_t x,uint8_t y,float num,uint8_t len,uint8_t size,uint8_t mode)
{
	...
	OLED_ShowNum(x+size,y,(uint32_t)temp,len,size);
	if(mode==0) return;
	temp*=10;
	OLED_ShowChar(x+(size*3),y,'.',size);
	temp=(uint32_t)temp%10;
	OLED_ShowNum(x+(size*3)+6,y,(uint32_t)temp,1,size);
}
...
{
	//显示pitch(俯仰)数值
	sprintf((char *)display_buf,"%.2f ",OutMpu.pitch);  
	OLED_ShowString(60,0,display_buf,16);
	//显示roll(翻滚)数值
	sprintf((char *)display_buf,"%.2f ",OutMpu.roll);  
	OLED_ShowString(60,2,display_buf,16);
	//显示yaw(偏航)数值
	sprintf((char *)display_buf,"%.2f ",OutMpu.yaw);  
	OLED_ShowString(60,4,display_buf,16);
	//显示 左右电机的速度值
	sprintf((char *)display_buf,"%d ",Encoder_left);
	OLED_ShowString(60,6,display_buf,16);	
	sprintf((char *)display_buf,"%d ",Encoder_right);
	OLED_ShowString(90,6,display_buf,16);	
